//! Midnight apply stage.
//!
//! Mirrors [`crate::sync::apply`] but is typed for [`MidnightDomainAdapter`]
//! instead of the Cardano [`DomainAdapter`].

use dolos_core::SyncExt;
use gasket::{framework::*, messaging::Message};
use tracing::debug;

use crate::{adapters::midnight::MidnightDomainAdapter, prelude::*};
...
#[derive(Stage)]
#[stage(name = "midnight_apply", unit = "WorkUnit", worker = "Worker")]
pub struct Stage {
    domain: MidnightDomainAdapter,
...
impl Stage {
    pub fn new(domain: MidnightDomainAdapter, housekeeping_interval: std::time::Duration) -> Self {
        Self {
            domain,
