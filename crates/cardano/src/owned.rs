use dolos_core::{BlockBody, BlockHash, BlockSlot, RawData, RawBlock, RawUtxoMap, TxoRef};
...
impl OwnedMultiEraOutput {
    pub fn decode(buf: Arc<RawData>) -> Result<Self, pallas::ledger::traverse::Error> {
        Self::try_new(buf, |x| {
            let RawData(era, cbor) = x.as_ref();
